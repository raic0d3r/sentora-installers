...
if [[ "$OS" = "CentOs" ]]; then
    ...
    $PACKAGE_INSTALLER sudo vim make zip unzip debconf-utils at build-essential bash-completion curl  zip gcc at make apt-transport-https lsb-release ca-certificates wget nano dbconfig-common sqlite3 postfix dovecot-imapd dovecot-lmtpd dovecot-pop3d dovecot-sqlite
fi
...
if [[ "$OS" = "CentOs" ]]; then
    ...
    $PACKAGE_INSTALLER php5.6 php5.6-gd php5.6-mbstring php5.6-intl php5.6-mysql php5.6-xml php5.6-xmlrpc php5.6-mcrypt php5.6-imap
    if [[ "$VER" != "10" || "$VER" != "9" ]]; then
		echo -e "\n# Building suhosin"
		if [[ "$OS" = "debian" ]]; then
...
if [[ "$OS" = "CentOs" ]]; then
    ...
    sed -i "s|;session.save_path = \"/var/lib/php/5.6\"|session.save_path = \"$PANEL_DATA/sessions\"|" $PHP_INI_PATH
fi
